  ...
  <div>
    ...
    <div v-for="(item,index) in GoodsShop" :key="index">
      <el-row class="Shop_Con">
        ...
        <el-col :md="10" :sm="12">
          <div class="Shop_Title">{{item.productName}}</div>
          <div class="Shop_pic">
            <p style="padding-top:10px; text-decoration:line-through;">
              原价：
              <span>{{item.oldprice|currency('&yen;')}}</span>
            </p>
            <span class="Shop_picl">
              促销价：
              <i>{{item.salePrice|currency('&yen;')}}</i>
            </span>
            <span class="sales">
              已销量：
              <i>{{item.sales}}</i>
          ...
          <div class="Shop_buy">
            <div class="btn-area">
              <a
                href="javascript:;"
                class="Shop_btn btn--m"
                style="background-color:#ef2f23;color:#FFF;width:40%;"
                @click="pay(item.productId)"
              >立即购买</a>
              <a
                href="javascript:;"
                class="Shop_btn btn--m"
          ...
          <div class="Shop_s">
            <p>
              支付方式：
              <img src="../../static/shop_pay.png" height="20" width="131" />
            </p>
          </div>
        </el-col>
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="商品详情" name="first">
            <el-col :md="24" class="Shop_text">
              ...
              <p>
                <img src="../../static/details.jpg" width="100%" />
              </p>
            </el-col>
          </el-tab-pane>
          <el-tab-pane label="查看评论" name="second">
            <marquee behavior direction width="100%">{{getDiscuss(discussLists)}}</marquee>
            <el-input
              type="textarea"
            ...
            ></el-input>
            <div class="action">
              <el-button type="primary" @click="createDiscuss()">提交评论</el-button>
            </div>
            <div class="discuss" v-for="(item,index) in discussLists" :key="index">
              <div>{{index+1}}&nbsp;{{item.discuss}}</div>
              <div>用户{{index+1}}</div>
            </div>
          </el-tab-pane>
        </el-tabs>
    ...
    <modal v-bind:mdShow="mdShow" v-on:close="closeMoadl">
      <p slot="message">请先登录,否则无法进行此操作!</p>
      <div slot="btnGroup">
...
export default {
  ...
  methods: {
    getDiscuss(discussLists) {
      let info = "";
      discussLists.forEach(element => {
        info += element.discuss + " ";
      });
      return info;
    },
    ...
    createDiscuss() {
      ...
      axios.post("/goods/discuss", discussInfo).then(res => {
    ...
    fetchDiscuss() {
      axios
        .get("/goods/discuss", {
          params: { productId: this.$route.query.shopid }
        })
        .then(res => {
          console.log(res.data);
          this.discussLists = res.data;
        });
    },
    init() {
      //初始化数据
